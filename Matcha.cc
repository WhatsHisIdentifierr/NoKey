...
RunService.RenderStepped:Connect(function()
    ...
    task.spawn(function()
        if not getgenv().Matcha.Legit.Triggerbot.Enabled then return end
        fireTrigger()
        if getgenv().Matcha.Legit.Triggerbot.Delay > 0 then
...
RunService.Heartbeat:Connect(function(dt)
    ...
    if useDesync and hrp then
        if matchacc.TargetAim.Strafe and matchacc.TargetAim.Target ~= "None" and TargetPlayer and TargetPlayer.Character and TargetPlayer.Character:FindFirstChild("Head") and not (AutoArmorActive or AutoLoadoutActive or BuyingSingleActive or BuyingAmmoActive or AutoHealActive) then
...
-- Logic AutoMask (tích hợp từ skido, với snap back)
coroutine.wrap(function()
    local RunService = game:GetService("RunService")
    local vu98 = {
...
RunService.Heartbeat:Connect(function()
	...
	if matchacc.Desync.Line then
	    ...
	    if onScreenRoot then
	        DesyncLine.From = mouseLocation
	        DesyncLine.To = Vector2.new(screenRoot.X, screenRoot.Y)
	        DesyncLine.Color = BodyCloneDesyncHighlight.FillColor or Color3.fromRGB(255, 255, 255)
	        DesyncLine.Visible = true
	    else
	        DesyncLine.Visible = false
	    end
